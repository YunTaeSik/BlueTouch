...
public class BluetoothLeService_Doorlock extends Service {
    private Context context = this;

    private final static String TAG = BluetoothLeService_Doorlock.class.getSimpleName();
    ...
    private final static String ACTION_GATT_CONNECTED =
            "com.example.bluetooth.le.ACTION_GATT_CONNECTED_DOORLOCK";
    private final static String ACTION_GATT_DISCONNECTED =
            "com.example.bluetooth.le.ACTION_GATT_DISCONNECTED_DOORLOCK";
    private final static String ACTION_GATT_SERVICES_DISCOVERED =
            "com.example.bluetooth.le.ACTION_GATT_SERVICES_DISCOVERED_DOORLOCK";
    private final static String ACTION_DATA_AVAILABLE =
            "com.example.bluetooth.le.ACTION_DATA_AVAILABLE_DOORLOCK";
    private final static String EXTRA_DATA_DOORLOCK =
            "com.example.bluetooth.le.EXTRA_DATA_DOORLOCK";
    private final BluetoothGattCallback mGattCallback = new BluetoothGattCallback() {
        ...
        @Override
        public void onConnectionStateChange(BluetoothGatt gatt, int status, int newState) {  //ble 상태 변경을 알리는 함수
            String intentAction;
        ...
        @Override
        public void onServicesDiscovered(BluetoothGatt gatt, int status) {  //ble 찾으면
    ...
    private void broadcastUpdate(final String action,
        ...
        final Intent intent = new Intent(action);
        System.out.println("BluetoothLeService_Doorlock broadcastUpdate");
        final byte[] data = characteristic.getValue();
        if (data != null && data.length > 0) {
            final StringBuilder stringBuilder = new StringBuilder(data.length);
            for (byte byteChar : data)
                stringBuilder.append(String.format("%02X ", byteChar));
            intent.putExtra(EXTRA_DATA_DOORLOCK, new String(data) +
    ...
    public class LocalBinder extends Binder {
        public BluetoothLeService_Doorlock getService() {
            return BluetoothLeService_Doorlock.this;
        }
    }
    ...
    public boolean initialize() {
        System.out.println("BluetoothLeService_Doorlock initialize" + mBluetoothManager);
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {  //서비스 시작
        Log.e("BluetoothLeService_Door", "BluetoothLeService_Doorlock");

        IntentFilter intentFilter = new IntentFilter();
        intentFilter.addAction(Contact.DOORLOCK_DATA);
        registerReceiver(mRecevier, intentFilter);  //인텐트 등록 및 브로드캐스트리시버 등록

        try {
            initBluetooth();  //스마트폰 블루투스 강제로 켜는 함수
            if (mBluetoothManager == null) {
                mBluetoothManager = (BluetoothManager) getSystemService(Context.BLUETOOTH_SERVICE);
            }
            mBluetoothAdapter = mBluetoothManager.getAdapter();
            try {
                address = SharedPrefsUtils.getStringPreference(getApplicationContext(), "doorlock_address");
        ...
        try {
            if (SharedPrefsUtils.getStringPreference(getApplicationContext(), Contact.Doorlock_Becon).equals(Contact.ON)) {
                 BeconMode();
            } else if (SharedPrefsUtils.getStringPreference(getApplicationContext(), Contact.Doorlock_Becon).equals(Contact.OFF)) {
                if (Data_Timer != null) {
                    Data_Timer.cancel();
                    Data_Timer = null;
                }
...
// 장치가 블루투스를 지원하는 경우
    ...
    private final BroadcastReceiver mGattUpdateReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (BluetoothLeService_Doorlock.ACTION_GATT_CONNECTED.equals(action)) {
            } else if (BluetoothLeService_Doorlock.ACTION_GATT_DISCONNECTED.equals(action)) {

            } else if (BluetoothLeService_Doorlock.ACTION_GATT_SERVICES_DISCOVERED.equals(action)) {
                try {
            ...
            } else if (BluetoothLeService_Doorlock.ACTION_DATA_AVAILABLE.equals(action)) {
                realAddress = intent.getExtras().getString("address");
                subData = intent.getExtras().getInt("data");
                if (mSCharacteristic == mModelNumberCharacteristic) {
                    try {
                        if (intent.getStringExtra(BluetoothLeService_Doorlock.EXTRA_DATA_DOORLOCK).toUpperCase().startsWith("DF BLUNO")) {
            ...
            } else if (action.equals("delete_doorlock")) {
                if (Data_Timer != null) {
                    Data_Timer.cancel();
                    Data_Timer = null;
                }
    ...
    private static IntentFilter makeGattUpdateIntentFilter() {
        ...
        intentFilter.addAction(BluetoothLeService_Doorlock.ACTION_GATT_CONNECTED);
        intentFilter.addAction(BluetoothLeService_Doorlock.ACTION_GATT_DISCONNECTED);
        intentFilter.addAction(BluetoothLeService_Doorlock.ACTION_GATT_SERVICES_DISCOVERED);
        intentFilter.addAction(BluetoothLeService_Doorlock.ACTION_DATA_AVAILABLE);
        intentFilter.addAction("delete_doorlock");
        return intentFilter;
    }
...
//                    updateConnectionState(R.string.comm_establish);
    ...
    private BroadcastReceiver mRecevier = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            if (intent.getAction().equals(Contact.DOORLOCK_DATA)) {
                ...
                String flag = intent.getStringExtra("DoorLock_Becon_flag");
                Log.e("DoorLock_DATA", String.valueOf(data) + flag);

                try {
                    if (flag.equals(Contact.ON)) {
                        BeconMode();
                    } else if (flag.equals(Contact.OFF)) {
                        if (Data_Timer != null) {
                            Data_Timer.cancel();
                            Data_Timer = null;
                        }
    ...
    protected static double calculateAccuracy(int txPower, double rssi) {  //rssi 와 tx power 로 거리재는 함수
        if (rssi == 0) {
            return -1.0;
        }
    ...
    private void BeconMode() {   //비콘 모드 켰을떄 작동하는 함수
        mTask = new TimerTask() {
            @Override
            public void run() {
                try {
                ...
                if (testDistance != 0) {
                    ...
                    if (count >= 20) {  //거리를 20번을 측정후 평균값을 구함
        ...
        TimerTask Data_task = new TimerTask() {
            ...
            public void run() {
                if (TheadHoldDistance == 0) {
                    if (averDistance / 4 <= 8) { //8m 안으로 들어오게 되면 측정 시작
                    ...
                    if (TheadHoldDistance < averDistance / 4) {
                        ...
                        if (filter >= 1) {
                            TheadHoldDistance = TheadHoldDistance + 0.5;  //1m 이상 멀어지면 0.5m만더함
                        } else {
                            TheadHoldDistance = averDistance / 4;
                        }
                        ...
                        if (filter >= 1) {
                            TheadHoldDistance = TheadHoldDistance - 0.5;  //1m 이상 가까워짐녀 0.5m만 뺌
                        } else {
                            TheadHoldDistance = averDistance / 4;
                        }
                    }
                    ...
                    if (TheadHoldDistance != 0) {
                        Log.e("distance_doorlock", String.valueOf(TheadHoldDistance));
                    }
                    if (TheadHoldDistance<= 1) {
                        Intent Light_Data_intent = new Intent(Contact.DOORLOCK_DATA);
                        Light_Data_intent.putExtra(Contact.data_doorlock, 1);
                        sendBroadcast(Light_Data_intent);
                    } else {
                        Intent Light_Data_intent = new Intent(Contact.DOORLOCK_DATA);
                        Light_Data_intent.putExtra(Contact.data_doorlock, 2);
                        sendBroadcast(Light_Data_intent);
                    }
                }
        ...
        Data_Timer.schedule(Data_task, 0, 3500);
    }
}
