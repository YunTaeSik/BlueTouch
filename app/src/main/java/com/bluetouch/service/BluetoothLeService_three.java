...
public class BluetoothLeService_three extends Service {
    private Context context = this;

    private final static String TAG = BluetoothLeService_three.class.getSimpleName();
    ...
    private final static String ACTION_GATT_CONNECTED =
            "com.example.bluetooth.le.ACTION_GATT_CONNECTED_THREE";
    private final static String ACTION_GATT_DISCONNECTED =
            "com.example.bluetooth.le.ACTION_GATT_DISCONNECTED_THREE";
    private final static String ACTION_GATT_SERVICES_DISCOVERED =
            "com.example.bluetooth.le.ACTION_GATT_SERVICES_DISCOVERED_THREE";
    private final static String ACTION_DATA_AVAILABLE =
            "com.example.bluetooth.le.ACTION_DATA_AVAILABLE_THREE";
    private final static String EXTRA_DATA_THREE =
            "com.example.bluetooth.le.EXTRA_DATA_THREE";
                ...
                if (status == BluetoothGatt.GATT_SUCCESS) {
                    ...
                    if (mCharacteristicRingBuffer.isEmpty()) {
                        ...
                        if (bluetoothGattCharacteristicHelper.mCharacteristicValue.length() > MAX_CHARACTERISTIC_LENGTH) {
                            ...
                            } else if (mBluetoothGatt.writeCharacteristic(bluetoothGattCharacteristicHelper.mCharacteristic)) {
                                System.out.println("writeCharacteristic init " + new String(bluetoothGattCharacteristicHelper.mCharacteristic.getValue()) + ":success");
                            } else {
    ...
    private void broadcastUpdate(final String action,
        ...
        final Intent intent = new Intent(action);
        System.out.println("BluetoothLeService_three broadcastUpdate");
        final byte[] data = characteristic.getValue();
        if (data != null && data.length > 0) {
            final StringBuilder stringBuilder = new StringBuilder(data.length);
            for (byte byteChar : data)
                stringBuilder.append(String.format("%02X ", byteChar));
            intent.putExtra(EXTRA_DATA_THREE, new String(data) +
    ...
    public class LocalBinder extends Binder {
        public BluetoothLeService_three getService() {
            return BluetoothLeService_three.this;
        }
    }
    ...
    public boolean initialize() {
        System.out.println("BluetoothLeService_three initialize" + mBluetoothManager);
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        try {
            Log.e("BluetoothLeService_thre", "BluetoothLeService_three");
            initBluetooth();
            if (mBluetoothManager == null) {
                mBluetoothManager = (BluetoothManager) getSystemService(Context.BLUETOOTH_SERVICE);
            }
            mBluetoothAdapter = mBluetoothManager.getAdapter();
            try {
                String add = SharedPrefsUtils.getStringPreference(context, "A" + 3);
                if (add != null) {
...
// 장치가 블루투스를 지원하는 경우
    ...
    private final BroadcastReceiver mGattUpdateReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (BluetoothLeService_three.ACTION_GATT_CONNECTED.equals(action)) {
            } else if (BluetoothLeService_three.ACTION_GATT_DISCONNECTED.equals(action)) {
            } else if (BluetoothLeService_three.ACTION_GATT_SERVICES_DISCOVERED.equals(action)) {
                try {
            ...
            } else if (BluetoothLeService_three.ACTION_DATA_AVAILABLE.equals(action)) {
                realAddress = intent.getExtras().getString("address");
                subData = intent.getExtras().getInt("data");
                if (mSCharacteristic == mModelNumberCharacteristic) {
                    try {
                        if (intent.getStringExtra(BluetoothLeService_three.EXTRA_DATA_THREE).toUpperCase().startsWith("DF BLUNO")) {
            ...
            } else if (action.equals("delete_three")) {
                disconnect();
                close();
            }
        }
    ...
    private static IntentFilter makeGattUpdateIntentFilter() {
        ...
        intentFilter.addAction(BluetoothLeService_three.ACTION_GATT_CONNECTED);
        intentFilter.addAction(BluetoothLeService_three.ACTION_GATT_DISCONNECTED);
        intentFilter.addAction(BluetoothLeService_three.ACTION_GATT_SERVICES_DISCOVERED);
        intentFilter.addAction(BluetoothLeService_three.ACTION_DATA_AVAILABLE);
        intentFilter.addAction("delete_three");
        return intentFilter;
    }
