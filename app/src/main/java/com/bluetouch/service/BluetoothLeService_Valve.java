...
public class BluetoothLeService_Valve extends Service {
    private Context context = this;

    private final static String TAG = BluetoothLeService_Valve.class.getSimpleName();
    ...
    private final static String ACTION_GATT_CONNECTED =
            "com.example.bluetooth.le.ACTION_GATT_CONNECTED_VALVE";
    private final static String ACTION_GATT_DISCONNECTED =
            "com.example.bluetooth.le.ACTION_GATT_DISCONNECTED_VALVE";
    private final static String ACTION_GATT_SERVICES_DISCOVERED =
            "com.example.bluetooth.le.ACTION_GATT_SERVICES_DISCOVERED_VALVE";
    private final static String ACTION_DATA_AVAILABLE =
            "com.example.bluetooth.le.ACTION_DATA_AVAILABLE_VALVE";
    private final static String EXTRA_DATA_VALVE =
            "com.example.bluetooth.le.EXTRA_DATA_VALVE";
    ...
    private void broadcastUpdate(final String action,
        ...
        final Intent intent = new Intent(action);
        System.out.println("BluetoothLeService_Valve broadcastUpdate");
        final byte[] data = characteristic.getValue();
        if (data != null && data.length > 0) {
            final StringBuilder stringBuilder = new StringBuilder(data.length);
            for (byte byteChar : data)
                stringBuilder.append(String.format("%02X ", byteChar));
            intent.putExtra(EXTRA_DATA_VALVE, new String(data) +
    ...
    public class LocalBinder extends Binder {
        public BluetoothLeService_Valve getService() {
            return BluetoothLeService_Valve.this;
        }
    }
    ...
    public boolean initialize() {
        System.out.println("BluetoothLeService_Valve initialize" + mBluetoothManager);
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        Log.e("BluetoothLeService_Va", "BluetoothLeService_Valve");
        ...
        try {
            ...
            mBluetoothAdapter = mBluetoothManager.getAdapter();
            try {
                address = SharedPrefsUtils.getStringPreference(getApplicationContext(), "valve_address");
        ...
        try {
            if (SharedPrefsUtils.getStringPreference(getApplicationContext(), Contact.Valve_Becon).equals(Contact.ON)) {
                BeconMode();
            } else if (SharedPrefsUtils.getStringPreference(getApplicationContext(), Contact.Valve_Becon).equals(Contact.OFF)) {
                if (Data_Timer != null) {
                    Data_Timer.cancel();
                    Data_Timer = null;
                }
...
// 장치가 블루투스를 지원하는 경우
    ...
    private final BroadcastReceiver mGattUpdateReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (BluetoothLeService_Valve.ACTION_GATT_CONNECTED.equals(action)) {
            } else if (BluetoothLeService_Valve.ACTION_GATT_DISCONNECTED.equals(action)) {

            } else if (BluetoothLeService_Valve.ACTION_GATT_SERVICES_DISCOVERED.equals(action)) {
                try {
            ...
            } else if (BluetoothLeService_Valve.ACTION_DATA_AVAILABLE.equals(action)) {
                realAddress = intent.getExtras().getString("address");
                subData = intent.getExtras().getInt("data");
                if (mSCharacteristic == mModelNumberCharacteristic) {
                    try {
                        if (intent.getStringExtra(BluetoothLeService_Valve.EXTRA_DATA_VALVE).toUpperCase().startsWith("DF BLUNO")) {
            ...
            } else if (action.equals("delete_valve")) {
                if (Data_Timer != null) {
                    Data_Timer.cancel();
                    Data_Timer = null;
                }
    ...
    private static IntentFilter makeGattUpdateIntentFilter() {
        ...
        intentFilter.addAction(BluetoothLeService_Valve.ACTION_GATT_CONNECTED);
        intentFilter.addAction(BluetoothLeService_Valve.ACTION_GATT_DISCONNECTED);
        intentFilter.addAction(BluetoothLeService_Valve.ACTION_GATT_SERVICES_DISCOVERED);
        intentFilter.addAction(BluetoothLeService_Valve.ACTION_DATA_AVAILABLE);
        intentFilter.addAction("delete_valve");
    ...
    private BroadcastReceiver mRecevier = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            if (intent.getAction().equals(Contact.VALVE_DATA)) {
                ...
                String flag = intent.getStringExtra("Valve_Becon_flag");
                Log.e("VALVE_DATA", String.valueOf(data) + flag);
                ...
                if (data == 1) {
                    mSCharacteristic.setValue(String.valueOf(3));
                    ...
                    try {
                        mSCharacteristic.setValue(String.valueOf(5));
                        mSCharacteristic.setWriteType(BluetoothGattCharacteristic.WRITE_TYPE_NO_RESPONSE);
                        mBluetoothGatt.writeCharacteristic(mSCharacteristic);
                    } catch (NullPointerException e) {
        ...
        TimerTask Data_task = new TimerTask() {
            ...
            public void run() {
                if (TheadHoldDistance == 0) {
                    ...
                    if (TheadHoldDistance != 0) {
                        Log.e("distance_valve", String.valueOf(TheadHoldDistance));
                    }
                    if ((int) (TheadHoldDistance) <= 3) {
                        Intent Light_Data_intent = new Intent(Contact.VALVE_DATA);
                        Light_Data_intent.putExtra(Contact.data_valve, 4);
                        sendBroadcast(Light_Data_intent);
                    } else {
                        Intent Light_Data_intent = new Intent(Contact.VALVE_DATA);
                        Light_Data_intent.putExtra(Contact.data_valve, 5);
                        sendBroadcast(Light_Data_intent);
                    }
                }
